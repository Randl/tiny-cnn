...
class bnn_threshold_layer : public layer<activation::identity> {
 public:
  ...
  bnn_threshold_layer(serial_size_t channels,
                      serial_size_t dim           = 1,
                      std::string binaryParamFile = "")
    : Base(dim * channels, dim * channels, 0, 0),
      dim_(dim),
      channels_(channels),
      thresholds_(channels, 0),
      invertOutput_(channels, false) {
  ...
  void loadFromBinaryFile(std::string fileName) {
    ...
    // --
    // bin weight files are generated by Python script that flips the weights
    // when
  ...
  size_t fan_in_size() const override { return dim_; }

  size_t fan_out_size() const override { return dim_; }
